...
% History
% 03/17/2017 Makoto. Changed default values.
...
g = arg_define([0 2],varargin, ...
    ...
    arg_subswitch({'footerPanelSpec','FooterPanelDisplaySpec'},'GraphMetric', ...
        {'off' { ...
    ...
    arg_sub({'BMopts','BrainMovieOptions'},[], ...
        { ...
            arg({'size','ImageSize'},[1200 1200],[],'Image size (pixels). Input should be [widthcond height]. If more than one condition is being plotted horizontally, then widthcond is the width of each condition subplot','cat','DisplayProperties') ...
            ...
            arg({'view','InitialView'},[90 0],[],'3D static starting view for the movie.  See ''help view''. Default is [50 36].','cat','DisplayProperties'), ...
            ...
            arg_sub({'graphColorAndScaling','GraphColorAndScaling'},{}, ...
                { ...
                arg({'nodeSizeLimits','NodeSizeLimits'},[0 0.9],[],'[Min Max] limits for node size (pixels).','shape','row','cat','DisplayProperties'), ...
                arg({'nodeColorLimits','NodeColorLimits'},[0 1],[],'[Min Max] limits for node color (colormap index).','shape','row','cat','DisplayProperties'), ...
                arg({'edgeSizeLimits','EdgeSizeLimits'},[0 0.9],[],'[Min Max] limits for edge size (pixels)','shape','row','cat','DisplayProperties'), ...
            ...
            arg_sub({'outputFormat','OutputFormat'},{}, ...   
                { ...
                arg({'framefolder','ImageOutputDirectory'},pwd,[],'Output directory to save images. If ''prompt'', then you will be prompted to select the folder from a dialog. If blank, don''t save images','shape','row','type','char'), ...
                arg({'framesout','ImageOutputFormat'},'jpg',{'jpg','eps','ppm','tif','ai','bmp','emf','pbm','pcx','pdf','pgm','png','fig'},'Format for saving images'), ...
                arg({'moviename','MovieOutputFilename'},'prompt',[],'Movie filename. E.g, ''movie.avi''. If ''prompt'', then you will be prompted to select the file from a dialog. If blank, don''t save movie.','shape','row','type','char'),...
...
if any(g.freqsToCollapse < freqrange(1)) || ...
   any(g.freqsToCollapse > round(freqrange(end))) % 03/17/2017 Makoto. freqrange(end) was slightly smaller than max(g.freqsToCollapse)
