...
% History
% 03/17/2017 Makoto. 
% 01/29/2016 Makoto. Updated the video.
% 01/21/2016 Steven Peterson. Show node labels when nodes are >0.
...
function [alltimepoints movieFrames g] = brainmovie3d_causal(ALLNODESIZE,ALLNODECOLOR,ALLEDGESIZE,ALLEDGECOLOR,TIMES,FREQS,SELECTED,varargin)
...
if ismember_bc(lower(g.mode),{'init','init_and_render'})
    ...
    % 01/13/2016 Makoto. Commented out this part because avifile() is old.
    % create movie
    % ------------
    if ~isempty(g.moviename)
        fprintf('A movie is being saved under %s (movie parameters shown below):',g.moviename);
        % movieFrames = avifile(g.moviename, g.movieopts{:});
    end
...
for indeximage = g.vars.alltimepoints
    ...
    for tmpcond=1:nbconditions
        ...
        if strcmpi(g.modulateEdgeSize,'on')
            ...
            tmpEdgeSizeMatrix = zeros(length(SELECTED), length(SELECTED));
            for index1 = SELECTED
                for index2 = SELECTED
                    ...
                    %%% Obtain tmpEdgeSize %%%
                    %%%%%%%%%%%%%%%%%%%%%%%%%%
                    tmpEdgeSizeMatrix(index1,index2) = tmpEdgeSize;
...
%         NumPointsCircum = 11; % num points around circumference of cylinder
        ...
        if ~isfield(g.vars,'hNodes') || isempty(g.vars.hNodes)
            ...
            %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
            %%% Hide node labels when node size is zero. by Makoto 03/17/2017. %%%
            %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
            % Update node surfaces.
            set(g.vars.hNodes,'Xdata',xs,'Ydata',ys,'Zdata',zs,'Cdata',colorarray,'vertexnormals',normals);
            nodeSizeScLabel=nodeSizeScaled; nodeSizeScLabel(nodeSizeScLabel<0)=0; nodeSizeScLabel(isnan(nodeSizeScLabel))=0;
            if (max(nodeSizeScLabel)~=1 && max(nodeSizeScLabel)~=0)
                nodeSizeScLabel=nodeSizeScLabel./max(nodeSizeScLabel);
            end
            
            % Update nodelabel position.
            if g.plotNodeLabels && ~g.vars.INIT_LABELS
                
                % First, delete all nodeTexts.
                set(g.vars.hNodeText, 'Visible', 'off')
                
                for index1 = 1:length(SELECTED)
                    
                    % Display nodeTexts only for non-negative nodes and edges.
                    if nodeSizeScLabel(index1)>0
                        % This is 'from'.
                        nodeCenterPos = g.vars.coordinates{tmpcond}{ index1 };
                        pos = get(g.vars.hNodeText(index1,1),'Position');
                        set(g.vars.hNodeText(index1,1),'Position',[pos(1:2) nodeCenterPos(1,3)+1.2*nodeSizeScaled(index1)],'Color', [nodeSizeScLabel(index1) nodeSizeScLabel(index1) nodeSizeScLabel(index1)],'Visible', 'on');
                    
                        % This is 'to'.
                        toRoiIdx = find(tmpEdgeSizeMatrix(:,index1)>0);
                        for toRoiIdxIdx = 1:length(toRoiIdx)
                            currentToRoiIdx = toRoiIdx(toRoiIdxIdx);
                            nodeCenterPos_To = g.vars.coordinates{tmpcond}{ currentToRoiIdx };
                            pos_To           = get(g.vars.hNodeText(currentToRoiIdx, 1),'Position');
                            set(g.vars.hNodeText(currentToRoiIdx,1),'Position',[pos_To(1:2) nodeCenterPos_To(1,3)+1.2*nodeSizeScaled(currentToRoiIdx)],'Color', [nodeSizeScLabel(index1) nodeSizeScLabel(index1) nodeSizeScLabel(index1)],'Visible', 'on');
                            % get(g.vars.hNodeText(currentToRoiIdx,1), 'String')
                        end
                    end
                    
                        % % handle dual dipole, if present--assuming this does not happen in groupSIFT.
...
%         end
    ...
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    %%% Generate movie by Makoto. 01/13/2016 %%%
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    movieFrames(indeximage) = getframe(g.figurehandle);
    
%     if ~isempty(g.moviename)
%         % save the file for a movie
%         % -------------------------
%         movframes = getframe(g.figurehandle);
%         movieFrames = addframe(movieFrames,movframes);
%     end
%     
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%% Generate movie by Makoto. 01/29/2016 %%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
if ~isempty(g.moviename)
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    % make a movie
    framefolderIdx = find(strcmp(varargin, 'framefolder'))+1;
    fileNameWithFullPath = [varargin{framefolderIdx} filesep g.moviename];
    writerObj = VideoWriter(fileNameWithFullPath);
    writerObj.FrameRate = 5;
    writerObj.Quality   = 95;
    open(writerObj)
    for numFrames = 1:length(movieFrames)
        writeVideo(writerObj,movieFrames(numFrames));
    end
    close(writerObj)
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
end
% if ~isempty(g.moviename)
%     movieFrames = close(movieFrames);
% end
